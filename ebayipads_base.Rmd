...
title: "eBay:iPads:: sold classification:: ebayipads_base"
author: "bdanalytics"
...
glbFeatsPrice <- NULL # c("startprice") : bid._sp # NULL or c("<price_var>")
...
glb_mdl_family_lst[["RFE.X"]] <- "glmnet" # non-NULL list is mandatory
...
if (glb_is_classification) {
    if (glb_is_binomial)
        glbMdlMetricsEval <- 
            c("max.AUCROCR.OOB", "max.Accuracy.OOB", "min.aic.fit", "max.Accuracy.fit") else
...
if (length(str_vars <- setdiff(str_vars, 
                               c(glbFeatsExclude, glb_txt_vars))) > 0) {
    for (var in str_vars) {
...
#stop(here"); glb_to_sav()    
    ...
    glb_txt_corpus_lst <- foreach(txt_var = glb_txt_vars, .verbose = TRUE) %dopar% {
    #glb_txt_corpus_lst <- foreach(txt_var = glb_txt_vars, .verbose = TRUE) %do% {        
    #for (txt_var in glb_txt_vars) {
...
myrun_rfe <- function(obs_df, indep_vars, sizes=NULL) {
    ...
    rfe_results <- rfe(rfe_obs_df[, predictors_vctr], 
                       ...
                       sizes=sizes, metric=unlist(strsplit(glbMdlMetricsEval, "[.]"))[2],
